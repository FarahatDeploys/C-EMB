// Author : Hady Ahmed Mohamed Farahat
// Data 6/11/2023
// This code solves the sales by match problem 
// link : 
...
int sockMerchant(int n, int ar_count, int* ar) {
    int totalPairs = 0;
    int totalRepeats = 0;
    int *arrTemp  = malloc(ar_count*sizeof(int));
    for(int z =0;z<ar_count;z++){
        arrTemp[z] = 0;
    }
    for(int i =0;i<ar_count;i++){
        int temp = ar[i];
        totalRepeats = 0;
        for(int j =i;j<ar_count;j++){
            if(ar[j]!=-1 && ar[j] == temp)
            {
        ...
        if(totalRepeats!=0){
            arrTemp[i] = totalRepeats +1;
        }
    }
    int numPairs = 0;
    for(int m = 0;m<ar_count;m++){
        numPairs = arrTemp[m]/2 +numPairs;
    }
    printf("Number of Pairs :%d",numPairs);
